...
function StatusBadge({ status, slaColor }: { status: string; slaColor: string | null }) {
  if (status === 'completed') {
    return (
    ...
    <div
      className={`rounded-xl border transition-colors ${
        stage.status === 'completed'
          ...
          : stage.status === 'blocked' || stage.sla_color === 'red'
          ? 'border-red-100 bg-red-50/30'
          : stage.sla_color === 'yellow'
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <StatusBadge status={stage.status} slaColor={stage.sla_color} />
          {hasDetails && (
            expanded ? (
